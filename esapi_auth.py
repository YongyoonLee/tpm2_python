...
# 해시 함수를 사용하기 위해, 외부 cryptography 라이브러리를 가져옵니다.
...
# TPM에서 8바이트의 무작위 데이터를 생성합니다.
r = ectx.get_random(8)
# 생성된 무작위 데이터를 출력합니다.
print(f"Created Random Data: {r}")
...
inSensitive = TPM2B_SENSITIVE_CREATE(
    TPMS_SENSITIVE_CREATE(userAuth=TPM2B_AUTH("password"))  # 사용자 인증을 위한 비밀번호를 설정합니다.
)
...
# SHA-256 해시 함수를 사용하여 데이터를 해시합니다.
digest = sha256(b"fff")
...
# 서명을 16진수로 출력합니다.
print(f"Signature: {signature.marshal().hex()}")
